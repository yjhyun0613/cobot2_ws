...
class VisionServerNode(Node):
    def __init__(self):
        ...
        try:
            ...
            self.gripper2cam_mat[:3, 3] = [0.0, 75.0, 39.91] # mm 단위
    ...
    def publish_static_transform(self):
        ...
        # 🌟 로드된 4x4 행렬에서 위치(mm) 및 회전값 추출
        trans_mm = self.gripper2cam_mat[:3, 3]
        rot_mat = self.gripper2cam_mat[:3, :3]
        quat = R.from_matrix(rot_mat).as_quat() # [x, y, z, w]

        # 🌟 핵심: ROS TF 트리는 m 단위로 동작해야 하므로 1000으로 나누어 m 단위로 퍼블리시
    ...
    def get_3d_point(self, u, v, depth_map, fx, fy, cx, cy, tf_matrix):
        # 🌟 카메라 depth는 mm이므로 m 단위로 변환하여 계산 후, 최종적으로 mm로 반환
        z = float(depth_map[v, u]) / 1000.0
        if z <= 0.1 or z > 2.0: return None # 0.1m ~ 2.0m
        x = (u - cx) * z / fx
        y = (v - cy) * z / fy
        pt_cam = np.array([x, y, z, 1.0]) # pt_cam is in m
        pt_base = tf_matrix @ pt_cam      # pt_base is in m
        return pt_base[:3] * 1000.0       # 최종 반환되는 계산 결과는 완벽한 mm 단위!
    ...
    def inspect_callback(self, request, response):
        ...
        for box in yolo_results.boxes:
            ...
            # 점 3개 기하학 분석 기반 나사의 법선 및 위치 연산 호출 (이미 mm 단위 도출)
            ...
            if normal is not None and center_3d is not None:
                ...
                # 2. 직교 평면 투영 정밀 단차(mm) 도출 (전체 식이 이미 mm 기반이므로 *1000 제거)
                vec_to_center = center_3d - plane_point
                delta_z_mm = abs(np.dot(vec_to_center, normal))
                
                # 3. 임계치 비교 합격 판정 및 위치 세팅 (이미 center_3d가 완벽한 mm 단위)
                status_bool = bool(delta_z_mm <= 10.0)
                pt_base_mm = center_3d 
        ...
        # 🌟 핵심: RealSense PC 데이터(m 단위)를 m 단위 그대로 사용
        pts_m = np.array([[p[0], p[1], p[2]] for p in pc_data])
        ...
        # 필터링 범위 (0.25m ~ 1.0m)
        distances = np.linalg.norm(pts_m, axis=1)
        mask = (distances >= 0.25) & (distances <= 1.0)
        pts_filtered, colors_filtered = pts_m[mask], colors[mask]
        
        # TF 행렬이 m 단위이므로, 변환 결과도 m 단위임
        pts_transformed_m = (tf_pc @ np.hstack([pts_filtered, np.ones((len(pts_filtered), 1))]).T).T[:, :3]
        pts_transformed_mm = pts_transformed_m * 1000.0 # 최종적으로 mm 단위로 변환

        bg_dict = {
            'x': np.round(pts_transformed_mm[:, 0], 2).tolist(),
            'y': np.round(pts_transformed_mm[:, 1], 2).tolist(),
            'z': np.round(pts_transformed_mm[:, 2], 2).tolist(),
