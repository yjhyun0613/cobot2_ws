...
try:
    from DSR_ROBOT2 import (
        ...
        task_compliance_ctrl, set_desired_force, release_compliance_ctrl, get_tool_force, get_current_posj
    )
...
class RobotScrewInspector(Node):
    ...
    def execute_screw_inspection_task(self, target_key="pos1"):
        ...
        try:
            ...
            self.get_logger().info("Step 5) 힘 제어 해제 및 나사 조이기 시작")

            # 1. 누르는 힘(Force) 및 순응 제어(Compliance) 강제 해제
            # 해제 시 로봇이 현재 위치(홈에 들어간 상태)에서 빳빳하게 고정됩니다.
            release_compliance_ctrl()
            time.sleep(0.5) # 제어권이 완전히 넘어올 때까지 짧게 대기

            # 2. 현재 관절(Joint) 각도 가져오기
            curr_joint = get_current_posj()
            target_joint = list(curr_joint)

            # 3. J6 축(6번째 관절) 회전 각도 설정
            # 조이고 싶은 만큼의 각도를 더해줍니다. (예: 360도 = 1바퀴)
            # 나사 종류나 길이에 따라 이 값을 수정하세요. (시계방향 +, 반시계방향 -)
            turn_degree = 90.0 
            target_joint[5] += turn_degree

            self.get_logger().info(f"J6 관절을 {turn_degree}도 회전하여 나사를 체결합니다.")

            # 4. 관절 회전 실행 (movej)
            # 나사가 물리적으로 박히는 시간을 고려해 속도(vel)를 30 정도로 낮추는 것이 좋습니다.
            movej(target_joint, vel=30, acc=30)
            mwait()
