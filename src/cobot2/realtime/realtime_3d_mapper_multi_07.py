...
class VisionServerNode(Node):
    ...
    # 🌟 mm 좌표계와 orientation 각도를 수용하도록 수정된 빌더 함수
    ...
    def analyze_screw_with_retry(self, bbox, depth_img, fx, fy, cx, cy, tf_mat):
        ...
        for attempt in range(5):
            ...
            if norm > 0:
                normal = normal / norm
                return normal, center_3d, b_3d # 법선, 중심점, 평면위의 점
    ...
    def inspect_callback(self, request, response):
        ...
        # 배경 포인트들도 mm 단위로 볼 경우 끝에 * 1000.0을 곱해주면 됩니다.
        # 기존 코드 호환을 위해 일단 m 단위를 유지합니다. 배경마저 mm로 보시려면 * 1000.0을 붙여주세요.
