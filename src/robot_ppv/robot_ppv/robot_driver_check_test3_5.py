...
class RobotScrewInspector(Node):
    ...
    def execute_screw_inspection_task(self, target_key="pos1"):
        ...
        try:
            ...
            # 5) 토크 검수 및 실시간 불량 판정
            # --------------------------------------------------------
            self.get_logger().info("Step 5) 누르는 힘 유지 상태로 회전하며 토크를 검수합니다.")

            # 1. 누르는 힘(Force) 인가 (순응 제어 상태 유지)
            # 나사 홈 이탈 방지를 위해 일정한 하향 압력 유지
            set_desired_force(fd=[0, 0, -10, 0, 0, 0], dir=[0, 0, 1, 0, 0, 0], mod=0)
            ...
            turn_degree = 90.0 
            target_joint[5] += turn_degree

            self.get_logger().info(f"J6 관절을 {turn_degree}도 회전하며 토크 측정을 시작합니다.")

            # 3. 비동기 회전 시작 (mwait()을 여기서 쓰지 않음)
            movej(target_joint, vel=15, acc=15) 

            # 4. 실시간 토크 모니터링 루프
            TORQUE_LIMIT = 5.0  # 임계값: 5N (실험치에 따라 조정)
            max_torque = 0.0
            is_defective = False

            # 회전 동작 중 실시간 감시 (약 2초간 0.1초 간격으로 측정)
            for _ in range(20):
                curr_force = get_tool_force()
                # 6축 힘 센서의 Z축 회전 토크(Mz) 확인
                current_torque = abs(curr_force[5]) 
                
                if current_torque > max_torque:
                    max_torque = current_torque

                # [불량 판정] 임계값 초과 시 즉시 동작 정지
                if current_torque > TORQUE_LIMIT:
                    stop()  # 로봇 팔 동작 강제 정지
                    self.get_logger().warn(f"⚠️ [불량 감지] 과토크 발생: {current_torque:.2f}N")
                    is_defective = True
                    break
            ...
            mwait() # 이동 명령 완료 대기
            release_compliance_ctrl() # 안전하게 해제

            # 5. 결과 판정 및 로깅
            if not is_defective:
                self.get_logger().info(f"✅ 정상 체결 (최대 토크: {max_torque:.2f}N)")
            else:
                self.get_logger().error(f"❌ 불량 체결 (과토크 발생)")
