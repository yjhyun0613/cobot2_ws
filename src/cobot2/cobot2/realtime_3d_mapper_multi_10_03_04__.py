...
class VisionServerNode(Node):
    def __init__(self):
        ...
        model_path = '/home/yoon/cobot2_ws/src/cobot2/resource/hyupdong2_yolo11x_realtest_corrected_best.pt'
        ...
        self.last_inspect_time_ms = None
        ...
        self.srv_new_session = self.create_service(Trigger, '/start_new_session', self.start_new_session_callback)
        self.get_logger().info('🤖 비전 서버 대기 중... (/vision_inspect, /start_new_session 서비스 로딩)')
    ...
    def reset_session(self):
        ...
        self.session_defect_count = 0
        self.capture_count = 0
        self.get_logger().info('🔄 [세션 리셋] 이전 세션 ID를 파괴하고 다음 검사시 새 폴더를 생성합니다.')

    def start_new_session_callback(self, request, response):
        self.get_logger().info('📢 외부 서비스 호출(/start_new_session)로 세션을 강제 초기화합니다.')
        self.reset_session()
        response.success = True
        response.message = "Session successfully reset"
        return response
    ...
    def ensure_session_started(self, now_ms):
        ...
        self.get_logger().info(f'🗂️ 새 검사 세션 폴더 생성 및 시작: {self.session_id}')
    ...
    def inspect_callback(self, request, response):
        ...
        # ⏱️ 마지막 검사 요청으로부터 60초 이상 경과 시 새 검사 주기라고 판단하여 세션 강제 자동 리셋
        if self.last_inspect_time_ms is not None:
            elapsed_sec = (now_ms - self.last_inspect_time_ms) / 1000.0
            if elapsed_sec > 60.0:
                self.get_logger().info(f'⏱️ 마지막 검사 후 장시간({elapsed_sec:.1f}초) 대기 감지 -> 새 세션을 위해 상태를 자동 리셋합니다.')
                self.reset_session()
        ...
        self.get_logger().info('📸 나사 발견! 정밀 분석 및 검증 시작...')
        ...
        # 작업대 이미지 상의 기하학적 순서(위에서 아래, 왼쪽에서 오른쪽)로 정렬
        filtered_boxes.sort(
            key=lambda item: (
        ...
        if not markers_data:
            self.get_logger().info('⏩ 유효한 나사가 검출되지 않았습니다. 검사를 저장하지 않고 건너뜁니다.')
            response.success = True
            response.message = "SKIPPED"
            return response

        # 유효한 나사가 정상 검출된 경우에만 작업대 번호를 부여하고 저장합니다.
        self.capture_count += 1
        section_name = f'작업대 {self.capture_count}'
        workstation_id = f'workstation_{self.capture_count:02d}'
        self.get_logger().info(f'💾 유효한 나사 검출 완료 -> [{section_name}] ({workstation_id})로 데이터 업로드 시작...')
        ...
        self.last_inspect_time_ms = now_ms # 유효한 검사가 성공적으로 완료되었을 때만 시간 갱신
