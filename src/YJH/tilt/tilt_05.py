...
class Yolo3DNormalNode(Node):
    ...
    def get_target_pose(self, center_point, normal_vector, standoff_distance=0.1):
        """중심점과 법선 벡터를 이용해 로봇의 목표 포즈(Pose) 계산"""
        z_axis = -np.array(normal_vector)
        z_axis = z_axis / np.linalg.norm(z_axis)

        x_axis_temp = np.array([1.0, 0.0, 0.0])
        if np.abs(np.dot(z_axis, x_axis_temp)) > 0.99:
            x_axis_temp = np.array([0.0, 1.0, 0.0])

        y_axis = np.cross(z_axis, x_axis_temp)
        ...
        approach_position = np.array(center_point) + (np.array(normal_vector) * standoff_distance)

        target_pose = Pose()
        target_pose.position.x = approach_position[0]
        target_pose.position.y = approach_position[1]
        target_pose.position.z = approach_position[2]
    ...
    def process_capture(self):
        ...
        for i, box in enumerate(results.boxes):
            ...
            if p_left is not None and p_right is not None and p_top is not None and p_center is not None:
                # 법선 벡터 계산
                normal = self.calculate_normal_vector(p_left, p_right, p_top)
                
                # 로봇 타겟 포즈 계산 (접근 거리 10cm 설정)
                target_pose = self.get_target_pose(p_center, normal, standoff_distance=0.1)

                self.get_logger().info(
                    f'\n🎯 [객체 {i+1} 타겟 포즈 계산 완료]\n'
                    f'   - 📍 객체 중심(X, Y, Z) : [{p_center[0]:.3f}, {p_center[1]:.3f}, {p_center[2]:.3f}]\n'
                    f'   - 📐 평면 법선(Nx, Ny, Nz): [{normal[0]:.3f}, {normal[1]:.3f}, {normal[2]:.3f}]\n'
                    f'   - 🌟 로봇 목표 위치 [10cm 상단 접근]:\n'
                    f'       (X: {target_pose.position.x:.3f}, Y: {target_pose.position.y:.3f}, Z: {target_pose.position.z:.3f})\n'
                    f'   - 🌟 로봇 목표 자세 [Quaternion]:\n'
                    f'       (x: {target_pose.orientation.x:.3f}, y: {target_pose.orientation.y:.3f}, z: {target_pose.orientation.z:.3f}, w: {target_pose.orientation.w:.3f})'
                )
            else:
                self.get_logger().warn(f'⚠️ 객체 [{i+1}] 깊이 데이터가 부족하여 포즈를 계산하지 못했습니다.')
