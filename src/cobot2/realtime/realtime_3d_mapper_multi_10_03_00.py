...
class VisionServerNode(Node):
    ...
    def get_protruding_centroid_in_bbox(self, bbox, depth_img, fx, fy, cx, cy, tf_mat, plane_point, normal, height_threshold=0.002):
        ...
        # 깊이 데이터 추출 및 3x3 미디언 블러 필터링 적용 (노이즈 제거)
        ...
        for v_idx in range(depth_patch.shape[0]):
            for u_idx in range(depth_patch.shape[1]):
                ...
                y = (v - cy) * z / fy
                pt_cam = np.array([x, y, z, 1.0])
                pt_base = tf_mat @ pt_cam
                pt_base = pt_base[:3]
                
                # 평면 법선 벡터 방향으로의 높이 계산
                height = np.dot(pt_base - plane_point, normal)
                
                # 평면 위로 돌출된(예: 2mm 이상) 포인트만 필터링 (최대 30mm로 아웃라이어 제어)
                if height_threshold <= height <= 0.03:
                    protruding_pts.append(pt_base)
                    
        if len(protruding_pts) >= 5:
            return np.mean(protruding_pts, axis=0)
    ...
    def analyze_screw_with_retry(self, bbox, depth_img, fx, fy, cx, cy, tf_mat):
        ...
        for attempt in range(5):
            ...
            if norm > 0:
                ...
                # 바운딩 박스 내의 실제 돌출된 포인트들의 무게중심 계산
                centroid_3d = self.get_protruding_centroid_in_bbox(bbox, depth_img, fx, fy, cx, cy, tf_mat, b_3d, normal)
                
                if centroid_3d is not None:
                    center_3d = centroid_3d
                else:
                    # 무게중심 산출 실패 시 기본 중심 픽셀 주변 평균값으로 폴백(Fallback)
