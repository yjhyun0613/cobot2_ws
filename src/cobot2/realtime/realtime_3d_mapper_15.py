...
class Yolo3DMapperNode(Node):
    def __init__(self):
        ...
        self.get_logger().info('★ 모든 데이터 수신 대기 중... "s" 키를 누르면 [동적 도넛 방식] 단차 검사가 시작됩니다. ★')
        ...
        try:
            ...
            while rclpy.ok():
                if select.select([sys.stdin], [], [], 0.1)[0]:
                    ...
                    if key.lower() == 's':
                        self.get_logger().info('📸 [탑다운 검사] 동적 도넛 알고리즘으로 진짜 바닥을 찾아 단차를 계산합니다...')
    ...
    def process_capture(self):
        ...
        for box in yolo_results.boxes:
            ...
            bw_img = int(xmax - xmin) # 바운딩 박스 가로 넓이
            bh_img = int(ymax - ymin) # 바운딩 박스 세로 높이

            screw_pts, surf_pts = [], []
            h, w = cv_depth.shape
            
            # 탐색 영역을 바운딩 박스 크기의 1.5배로 동적 확장
            ...
            for dy in range(-search_ry, search_ry + 1):
                for dx in range(-search_rx, search_rx + 1):
                    ...
                    if 0 <= nx < w and 0 <= ny < h:
                        ...
                        if z_mm > 0:
                            # 1. 나사 머리: 바운딩 박스 중심부 20% 이내 (가장 높이 솟은 코어 부분)
                            if abs(dx) < bw_img * 0.1 and abs(dy) < bh_img * 0.1:
                                screw_pts.append(z_mm)
                            # 2. 진짜 바닥면: 바운딩 박스 바깥쪽 안전지대 (크기의 60% 바깥쪽)
                            elif abs(dx) > bw_img * 0.5 or abs(dy) > bh_img * 0.5:
                                surf_pts.append(z_mm)

            if not screw_pts or not surf_pts: 
                self.get_logger().warn(f'나사 또는 바닥면 깊이 측정 실패 (좌표: {cx_img}, {cy_img})')
                continue
            
            # 단차 계산 (나사 머리는 가장 높은 곳, 바닥은 중앙값)
            z_screw, z_surf = max(screw_pts), np.median(surf_pts)
            delta_z = (z_screw - z_surf) / 1000.0
            
            # 판정: 5mm(0.005m) 이상 단차가 나면 불량
            is_defective = delta_z > 0.005 
            
            status = "불량(미체결)" if is_defective else "정상"
            ...
            self.get_logger().info(f'🎯 [{status}] 단차: {delta_z*1000:.1f}mm | Z_나사: {z_screw:.1f}mm, Z_바닥: {z_surf:.1f}mm')
        ...
        fig.update_layout(scene=dict(aspectmode='data'), title=f"동적 도넛 단차 검사 결과: {self.capture_count}")
        fig.write_html(f'inspection_{self.capture_count}.html')
        cv2.imwrite(f'inspection_{self.capture_count}.jpg', yolo_results.plot())
        self.get_logger().info('✅ 동적 검사 완료! 단차가 5mm를 넘는지 확인하세요.')
