...
class Yolo3DMapperNode(Node):
    def __init__(self):
        ...
        self.get_logger().info('★ 모든 데이터 수신 대기 중... "s" 키를 누르면 "good/ng" 판정 검사가 시작됩니다. ★')
        ...
        try:
            ...
            while rclpy.ok():
                if select.select([sys.stdin], [], [], 0.1)[0]:
                    ...
                    if key.lower() == 's':
                        self.get_logger().info('📸 [검사 시작] AI 모델이 나사를 판정합니다...')
    ...
    def process_capture(self):
        ...
        # 1. 정밀 검사 로직 (YOLO 라벨 활용)
        for box in yolo_results.boxes:
            b = box.xyxy[0].cpu().numpy()
            class_id = int(box.cls[0].cpu().numpy())
            label = self.yolo_model.names[class_id] # 'good' or 'ng'

            cx_img = int((b[0] + b[2]) / 2)
            cy_img = int((b[1] + b[3]) / 2)

            # 높이 측정 (기존 로직 유지하여 데이터 검증용으로 활용)
            search_radius = 7
            screw_pts, surf_pts = [], []
            h, w = cv_depth.shape
            ...
            if not screw_pts: continue
            
            # 레이블에 따른 색상 결정
            color = 'red' if label == 'ng' else 'green'
            z_screw = max(screw_pts)
            ...
            # 로그 출력
            self.get_logger().info(f'🎯 [AI 판정] {label.upper()} -> base_link 좌표: X={pt_base[0]:.3f}, Y={pt_base[1]:.3f}')
            detected_markers.append({'name': f"{label}", 'x': pt_base[0], 'y': pt_base[1], 'z': pt_base[2], 'color': color})
        ...
        fig.update_layout(scene=dict(aspectmode='data'), title=f"AI 검사 결과: {self.capture_count}")
        fig.write_html(f'inspection_{self.capture_count}.html')
        cv2.imwrite(f'inspection_{self.capture_count}.jpg', yolo_results.plot())
        self.get_logger().info('✅ AI 판정 완료! 결과 파일을 확인하세요.')
