...
class Yolo3DMapperNode(Node):
    def __init__(self):
        ...
        self.get_logger().info('★ 모든 데이터 수신 대기 중... "s" 키를 누르면 [높이 기반] 단차 검사가 시작됩니다. ★')
        ...
        try:
            ...
            while rclpy.ok():
                if select.select([sys.stdin], [], [], 0.1)[0]:
                    ...
                    if key.lower() == 's':
                        self.get_logger().info('📸 [탑다운 검사] 나사의 위치와 단차(높이)만으로 불량을 판정합니다...')
    ...
    def process_capture(self):
        ...
        # 1. 오직 "높이(단차)"로만 판정하는 로직
        for box in yolo_results.boxes:
            b = box.xyxy[0].cpu().numpy()
            
            # 욜로의 라벨(good/ng)은 무시하고 오직 위치(Bounding Box)만 사용합니다.
            cx_img = int((b[0] + b[2]) / 2)
            cy_img = int((b[1] + b[3]) / 2)

            # 도넛 탐색 (나사 머리 vs 바닥면 높이 계산)
            search_radius = 7
            screw_pts, surf_pts = [], []
            h, w = cv_depth.shape
            ...
            for dy in range(-search_radius, search_radius + 1):
                for dx in range(-search_radius, search_radius + 1):
                    ...
                    if 0 <= nx < w and 0 <= ny < h:
                        ...
                        if z_mm > 0:
                            if abs(dx) < 3 and abs(dy) < 3: screw_pts.append(z_mm)
                            else: surf_pts.append(z_mm)
            ...
            # 🌟 [오직 여기서만 상태 판정] 5mm 이상 단차가 나면 무조건 불량!
            ...
            # 로그 출력
            self.get_logger().info(f'🎯 [높이 검사] {status} -> 단차: {delta_z*1000:.1f}mm, base_link 좌표: X={pt_base[0]:.3f}, Y={pt_base[1]:.3f}')
            
            # 마커 이름은 심플하게 통일 (good/ng 글자 대신 상태 출력)
        ...
        fig.update_layout(scene=dict(aspectmode='data'), title=f"탑다운 높이 검사 결과: {self.capture_count}")
        fig.write_html(f'inspection_{self.capture_count}.html')
        cv2.imwrite(f'inspection_{self.capture_count}.jpg', yolo_results.plot())
        self.get_logger().info('✅ 높이 검사 완료! HTML 결과파일을 확인하세요.')
