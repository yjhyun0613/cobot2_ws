...
# 👇 [추가] pose1, pose2, pose3의 목적지 좌표 정의 (예시 값입니다)
POSE_1 = [500.0, -200.0, 174.4, 156.4, 180.0, -112.5] 
POSE_2 = [500.0, -100.0, 174.4, 156.4, 180.0, -112.5] 
POSE_3 = [500.0,   0.0, 174.4, 156.4, 180.0, -112.5]
...
class RobotController(Node):
    ...
    def robot_control(self):
        ...
        if get_keyword_result and get_keyword_result.success:
            ...
            # 👇 [핵심 추가/수정] 놓을 위치(Pose)와 잡을 물체 분리하기
            drop_pos = BUCKET_POS  # 예외 처리: 목적지가 없으면 기본값인 BUCKET_POS 적용
            objects_to_pick = []   # YOLO로 보낼 진짜 물체들만 담을 리스트
            ...
            for target in target_list:
                ...
                # 목적지 단어인지 확인 후 drop_pos 설정
                if clean_target == "pose1":
                    drop_pos = POSE_1
                elif clean_target == "pose2":
                    drop_pos = POSE_2
                elif clean_target == "pose3":
                    drop_pos = POSE_3
                else:
                    # 목적지 단어가 아니면 잡아야 할 물체 리스트에 추가
                    objects_to_pick.append(clean_target)
                ...
                if target_pos is None:
                    ...
                    self.pick_and_place_target(target_pos, drop_pos)
                    self.init_robot()
        else:
    ...
    def pick_and_place_target(self, target_pos, drop_pos):
  ...
  ### 재권추가시작
        ...
        # 놓을 위치로 이동
# 👇 [수정] BUCKET_POS 대신 음성으로 지정된(혹은 예외처리된) drop_pos로 이동
        movel(drop_pos, vel=VELOCITY, acc=ACC)
        mwait()
        gripper.open_gripper()
