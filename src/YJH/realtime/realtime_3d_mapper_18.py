...
class Yolo3DMapperNode(Node):
    ...
    def process_capture(self):
        ...
        # --- 1m 이내 포인트 클라우드 필터링 로직 ---
        # 카메라 원점에서 포인트까지의 거리 계산 (x^2 + y^2 + z^2 의 제곱근)
        distances = np.linalg.norm(pts, axis=1)
        
        # 거리가 1.0m 이하인 점들만 True로 마스킹
        mask = distances <= 1.0
        
        # 마스크를 적용하여 1m 이내의 점과 색상만 남김
        pts_filtered = pts[mask]
        colors_filtered = colors[mask]
        
        # 필터링된 점들을 베이스 링크(Base Link) 좌표계로 변환
        pts_transformed = (tf_pc @ np.hstack([pts_filtered, np.ones((len(pts_filtered), 1))]).T).T[:, :3]
        
        # 필터링된 데이터로 Plotly 그래프 생성
        plot_data = [go.Scatter3d(x=pts_transformed[:, 0], y=pts_transformed[:, 1], z=pts_transformed[:, 2], 
                                  mode='markers', marker=dict(size=1.5, color=[f'rgb({int(c[0]*255)},{int(c[1]*255)},{int(c[2]*255)})' for c in colors_filtered]))]
        ...
        fig.update_layout(scene=dict(aspectmode='data'), title=f"10mm 기준 단차 검사 결과: {self.capture_count} (1m 이내 데이터)")
