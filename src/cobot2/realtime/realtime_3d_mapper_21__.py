...
# --- Firebase 설정 변수 ---
SERVICE_ACCOUNT_KEY_PATH = '/home/yoon/JoinGo/YJH/resource/rokey-d-2-4c32a-firebase-adminsdk-fbsvc-8e6d802500.json'
...
class Yolo3DMapperNode(Node):
    def __init__(self):
        ...
        model_path = '/home/yoon/JoinGo/YJH/resource/hyupdong2_yolo11x_img960_best.pt'
        self.get_logger().info(f'YOLO 모델 로딩 중... ({model_path})')
        self.yolo_model = YOLO(model_path)
        self.get_logger().info('✅ YOLO 모델 로딩 완료!')
        
        # --- 2-1. Firebase 초기화 (Storage + Database) ---
        try:
        ...
        self.get_logger().info('★ 데이터 수신 대기 중... "s" 키를 누르면 [검사 및 데이터 전송]이 시작됩니다. ★')
    ...
    def init_capture_count(self):
        try:
            ...
            if snapshot:
                keys = [int(k) for k in snapshot.keys() if k.isdigit()]
                if keys:
                    self.capture_count = max(keys)
                    self.get_logger().info(f'🔄 기존 데이터 확인됨. 현재 {self.capture_count}회차부터 이어집니다.')
                    return
            self.capture_count = 0
        except Exception as e:
            self.get_logger().error(f'기존 회차 조회 실패로 0부터 시작합니다: {e}')
            self.capture_count = 0
        ...
        try:
            ...
            while rclpy.ok():
                if select.select([sys.stdin], [], [], 0.1)[0]:
                    ...
                    if key.lower() == 's':
                        self.get_logger().info('📸 [검사 진행] 데이터를 추출하여 Firebase로 전송합니다...')
    ...
    def process_capture(self):
        ...
        for box in yolo_results.boxes:
            ...
            # 🚀 수정됨: 25cm(0.25m) 미만이거나 1m 초과면 건너뛰기
            ...
            markers_data[str(screw_idx)] = {
                "status": status_bool,
                "position": {
                    "x": float(pt_base[0]*1000),
                    "y": float(pt_base[1]*1000),
                    "z": float(pt_base[2]*1000)
                },
                "time": time_display_str
            }
            screw_idx += 1
            self.get_logger().info(f'🎯 나사 {screw_idx-1} 결과 -> {"정상" if status_bool else "불량(돌출)"} (단차: {delta_z_mm:.1f}mm)')

        # 2. 배경 포인트 클라우드를 .js 스크립트 파일 형태로 변환 및 로컬 저장
        self.get_logger().info('☁️ 0.25m ~ 1m 이내 포인트 클라우드 필터링 중...')
        ...
        # 🚀 수정됨: 거리가 0.25m 이상이고 1.0m 이하인 데이터만 마스킹
        ...
        self.get_logger().info('🚀 Firebase 동기화 데이터를 업로드합니다...')
        ...
        if public_url and self.db_ref:
            self.db_ref.child(str(self.capture_count)).set({
                "background_url": public_url,
                "markers": markers_data
            })
            self.get_logger().info(f'🎉 [{self.capture_count}회차] 데이터 동기화 완료! 웹 대시보드 확인 가능.')
        else:
