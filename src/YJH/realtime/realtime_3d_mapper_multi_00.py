...
class MapStitcherNode(Node):
    def __init__(self):
        ...
        self.get_logger().info('🗺️ 3D 맵 스티칭 모드 준비 완료!')
    ...
    def finish_and_save_map(self):
        ...
        # 0.005 = 5mm 간격으로 점들을 깔끔하게 정리 (맵 퀄리티 결정)
        ...
        fig.update_layout(scene=dict(aspectmode='data', 
                          ...
                          title=f"Stitched 3D Map (Total {self.capture_count} scans)")
        ...
        # 저장 후 메모리 초기화 (원하면 계속 이어서 다른 맵을 만들 수 있음)
        self.accumulated_points = []
        self.accumulated_colors = []
        self.capture_count = 0
