...
class Yolo3DNormalNode(Node):
    ...
    def load_yolo_model(self):
        """백그라운드에서 YOLO 모델을 로딩하여 메인 스레드 멈춤을 방지"""
        try:
    ...
    def process_capture(self):
        ...
        for i, box in enumerate(results.boxes):
            ...
            # 중심, 중심에서 위로, 중심에서 우측으로 이동된 픽셀값 확보
            ...
            if p_center is not None and p_top is not None and p_right is not None:
                ...
                self.get_logger().info(f'🎯 객체 [{i+1}] 발견! -> 중심 3D 좌표: {np.round(p_center, 3)}, 법선 벡터(Normal): {np.round(normal, 3)}')
            else:
                self.get_logger().warn(f'⚠️ 객체 [{i+1}] 깊이(Depth) 데이터가 불안정하여 법선 벡터를 계산하지 못했습니다.')
